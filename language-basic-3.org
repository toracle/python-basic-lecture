...
mode에 대해 설명. rt, rb, wt, wb 등. encoding 옵션도.
...
의미망 예제가 조금 복잡했었죠? 그걸 함수를 사용해서 묶어봅시다.
...
#+BEGIN_SRC python :results output :exports both
  class Speech:
     ...
     def draw(self, cutoff=2):
        wordnet = construct_wordnet(self.content)
        remove_low_frequency(wordnet, cutoff=cutoff)
        draw_graph(wordnet)
#+END_SRC
