...
앞서 의미망에서 사용한 함수 중, 아래 함수는 아직도 상당히 복잡해보입니다. 조금 더 작고 간단한 함수들로 쪼개보세요.
...
#+BEGIN_SRC python :exports none :results output
  def clean_words(words):
      cleansed_words = [w.replace('.', '').replace(',', '').strip() for w in words]
      cleansed_words_2 = [w for w in cleansed_words if len(w) > 1]
