...
Anaconda에서 기본적으로 제공되는 Spyder라는 편집기를 사용해서 파이썬 파일을 작성해봅시다.
...
=helloworld.py= 라는 파일을 만들고, 아래와 같이 입력하세요.
...
ex. 위 코드를 변경해서 1~50까지의 숫자를 출력해보세요.

=while= 문은 조건이 참일 동안 블럭 안의 내용을 반복해서 수행합니다.
...
#+BEGIN_EXAMPLE
Python에서는 공백이 매우 중요합니다. 같은 층위의 코드들은 공백이나 탭의 갯수가 동일해야 합니다. 공백이나 탭의 갯수가 서로 일치하지 않는 경우, 'IndentationError: unexpected indent' 오류가 발생합니다.
#+END_EXAMPLE
