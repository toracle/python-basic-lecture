...
a_list[2]            # 특정 위치의 원소 참조. 참조번호는 0번부터 시작.
a_list[2:4]          # 참조번호 2부터 4까지를 잘라서 반환함. 원본은 변경하지 않고 복사본을 반환.
1 in a_list          # 1이라는 값이 리스트에 존재하는지 여부를 반환
...
[['Tom', 'M', 15, True], 
 ['Chris', 'F', 28, True], 
 ['Timothy', 'M', 32, False]]                    # 리스트로 이루어진 리스트 (2)
#+END_SRC
